		...
		if wb_ini == []:
			...
			if (self.nd > 1):
				for i in range(1,self.nd):
					wb.append((np.random.rand(self.hiddenunits[0][i - 1] + 1, self.hiddenunits[0][i]) * 2 - 1) * 0.1)
			
			wb.append((np.random.rand(self.hiddenunits[0][-1] + 1, self.output_unit) * 2 - 1) * 0.1)
			self.wb = wb
		else:
			self.wb = wb_ini
